...
(function (Aufgabe7) {
    ...
    let gesammtpreis = document.createElement("a");
    ...
    function f_divs_generieren() {
        console.log(localStorage.getItem("anzahlArtikel"));
        //console.log(produkte.length.toString());
        for (let x = 0; x <= (parseInt(localStorage.getItem("anzahlArtikel")) - 1); x++) {
    ...
    function löschenArtikel(_element) {
        let target = _element.target;
        let preisbutton = target.innerHTML.substring(10);
    ...
    function löschenAlles(_element) {
        for (let x = 0; x <= (parseInt(localStorage.getItem("anzahlArtikel")) - 1); x++) {
